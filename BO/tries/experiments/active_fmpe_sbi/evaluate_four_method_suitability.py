...
def add_run_metadata(
    ...
    return add_objective_fields(row, args)
...
def add_objective_fields(
    row: dict[str, float | int | str],
    args: argparse.Namespace,
) -> dict[str, float | int | str]:
    row = dict(row)
    row["reward_mode"] = row.get("reward_mode", "") or args.reward_mode
    row["coverage_weight"] = row.get("coverage_weight", "") or args.coverage_weight
    row["predictive_weight"] = row.get("predictive_weight", "") or args.predictive_weight
    if row.get("posterior_quality_objective") in {"", None}:
        row["posterior_quality_objective"] = compute_posterior_quality_objective(
            row,
            args.reward_mode,
...
def ensure_diagnostics_schema(path: Path, args: argparse.Namespace) -> None:
    if not path.exists():
        return
    with path.open(newline="") as f:
    ...
    rows = [add_objective_fields(row, args) for row in load_existing_rows(path)]
...
def plot_summary(summary_rows: list[dict[str, float | int | str]], output: Path) -> None:
    ...
    panels = [
        ...
        ("posterior_quality_objective_mean", "BO-style objective\n(higher is better)"),
    ...
    fig.suptitle("Approach 1.2: Four-Method Suitability Check", fontsize=14)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--design-space", choices=("full", "window", "hard_window", "wide_window"), default="hard_window")
    ...
    parser.add_argument(
        "--reward-mode",
        choices=("rmse_coverage", "rmse_coverage_predictive"),
        default="rmse_coverage_predictive",
        help="Shared objective used to align summary metrics with the BO diagnostic.",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).parents[1] / "results" / "lotka_volterra" / "approach_1_2_four_method_suitability_check",
    )
...
def main() -> None:
    ...
    ensure_diagnostics_schema(diagnostics_path, args)
