# Approach 1.1 Active Design Snapshot

This folder copies the existing BO-vs-random active-design outputs from:

```text
experiments/active_fmpe_sbi/
```

Included output bundles:

```text
outputs_hard_window_long/
outputs_fmpe_full_design/
outputs_fmpe_full_design_long/
outputs_fmpe_hard_window/
```

These are the pre-diagnostic results used to motivate the later suitability and BO-design-effect checks.
