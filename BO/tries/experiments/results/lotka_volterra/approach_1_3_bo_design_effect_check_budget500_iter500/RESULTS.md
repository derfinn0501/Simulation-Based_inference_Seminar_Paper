...
## Comparison To `fmpe_max_iter = 220`

This run changes only the FMPE optimizer iteration cap relative to
`approach_1_3_bo_design_effect_check_budget500_round30`.

The final-round metrics are unchanged from the `fmpe_max_iter = 220` run. Only
one intermediate random-design row changed slightly, so this diagnostic does
not support increasing `fmpe_max_iter` alone as the next BO improvement lever.
